...
def main():
    # Paths
    model_path = r'experiments\PCB_product2_nano\weights\best.pt'
    images_dir = r'C:\data\product2_yolo\test\images'
    labels_dir = r'C:\data\product2_yolo\test\labels'
    output_dir = r'experiments\PCB_product2_nano\results'
    ...
    for img_path in image_files:
        ...
        # Draw Pred in Red (or Blue)
        for box in pred_boxes:
            cv2.rectangle(img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (0, 0, 255), 2)
            cv2.putText(img, f'Pred', (int(box[0]), int(box[1])-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
    ...
    print(f"F1 Score (conf={conf_threshold}): {f1:.4f}")
    print("-" * 30)
