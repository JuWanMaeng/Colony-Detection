...
def infer_multiscale(model, img, overlap_ratio=0.2):
    ...
    for cfg in STAGE_CONFIGS:
        preds = infer_grid_with_overlap(
            model, img, cfg["rows"], cfg["cols"], cfg["conf"], cfg["iou"], cfg["max_det"], overlap_ratio
        )
        merged += preds
...
# ====================================================
def evaluate_method_per_class(model, img_paths, labels_dir, class_ids, method="full", iou_thr=0.5, overlap_ratio=0.2):
    ...
    for cid in class_ids:
        ...
        print(f"[Class {cid}] TP={TP}, FP={FP}, FN={FN} → P={P:.4f}, R={R:.4f}, F1={F1:.4f}")
