...
# =============================
# 🔥 YOLO 포맷 TXT 저장 함수 (정규화 + conf 포함)
# =============================
def save_txt(preds, img_path, mode, txt_out_dir="total_results_txt"):
    ...
    name = os.path.splitext(os.path.basename(img_path))[0]
    txt_path = os.path.join(txt_out_dir, f"{name}_{mode}.txt")

    with open(txt_path, "w") as f:
        for cls, x1, y1, x2, y2, conf in preds:
            # YOLO 포맷 (cx, cy, w, h) 정규화
            cx = ((x1 + x2) / 2) / W
            cy = ((y1 + y2) / 2) / H
            w = (x2 - x1) / W
            h = (y2 - y1) / H
...
# 🔥 시각화 + TXT 저장 함수
# =============================
def draw_and_save(img_path, preds, mode, out_dir="total_results", txt_out_dir="total_results_txt", cut_ratio=False):
    os.makedirs(out_dir, exist_ok=True)
    img = cv2.imread(img_path)
    if img is None:
        print("⚠️ 이미지 로드 실패:", img_path)
        return
    ...
    # ✅ TXT 저장 호출
    save_txt(preds, img_path, mode, txt_out_dir)
    ...
    for cls, x1, y1, x2, y2, conf in preds:
        ...
        if label:
            cv2.putText(
                vis, f"{label} {conf:.2f}", (int(x1), max(15, int(y1) - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1
            )
...
# =============================
# 전체 이미지 처리 (Full / MS / Isolated + TXT 저장 포함)
# =============================
def process_all_images(model_path, dataset_root, overlap_ratio=0.2, cut_ratio=False):
    ...
    for img_path in tqdm(images, desc="Processing Images"):
        ...
        draw_and_save(
            ...
            full_preds,
            mode="full",
            out_dir="total_results",
            txt_out_dir="total_results_txt",
            cut_ratio=cut_ratio,
        )
        ...
        ms_preds = infer_multiscale(model, img, overlap_ratio=overlap_ratio)
        draw_and_save(
            img_path, ms_preds, mode="ms", out_dir="total_results", txt_out_dir="total_results_txt", cut_ratio=cut_ratio
        )

        # 3) Isolated COLONY Only Prediction
    ...
    print("✅ 완료! 시각화는 total_results 폴더, YOLO txt는 total_results_txt 폴더에 저장되었습니다.")
