...
def infer_multiscale(model, img, stage_configs, merge_nms_iou=0.5, max_keep=5000):
    ...
    for cfg in stage_configs:
        merged += infer_grid(
            model, img, cfg["rows"], cfg["cols"], cfg["conf"], cfg["iou"], max_det=cfg.get("max_det", 3000)
        )
        ...
        for sc in scenarios:
            ...
            writer.writerow(
                [
                    ...
                    sc["type"],
                    len(img_list),
                    round(latency_ms_avg, 2),
                    round(latency_ms_p50, 2),
                    round(latency_ms_p90, 2),
                    round(latency_ms_p95, 2),
            ...
            print(f"\n[Summary] {sc['name']}")
            print(
                f"  Latency avg/p50/p90/p95 (ms): {latency_ms_avg:.2f} / {latency_ms_p50:.2f} / {latency_ms_p90:.2f} / {latency_ms_p95:.2f}"
            )
